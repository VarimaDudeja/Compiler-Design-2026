...
AnnotatedNode *analyseAST(ASTNode *node) {
    ...
    /* ── PRINT  (PRINT expr) ─────────────────────────────── */
    case NODE_PRINT: {
        AnnotatedNode *expr = analyseAST(node->left);
        an->left = expr;
        if (expr && expr->dataType == TYPE_ERROR) {
            an->dataType = TYPE_ERROR;
        } else {
            an->dataType = TYPE_INT;  // print doesn't return a value, but for consistency
        }
        break;
    }
    ...
    /* ── PROGRAM ──────────────────────────────────────────── */
    case NODE_PROGRAM:
        an->left = analyseAST(node->left);
        break;

    /* ── STATEMENT LIST ───────────────────────────────────── */
    case NODE_STATEMENT_LIST:
        an->left = analyseAST(node->left);
        an->right = analyseAST(node->next);
        break;
    }
...
void printAnnotatedAST(AnnotatedNode *an, int level) {
    ...
    case NODE_STATEMENT_LIST:
        printAnnotatedAST(an->left, level + 1);
        if (an->right) printAnnotatedAST(an->right, level);
        break;
    }
}
